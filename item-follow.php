...
// $sql = sprintf("INSERT INTO `followed_list`
// (`items_sid`, `user_sid`, `followed_at`) 
// VALUES (%s, %s, NOW())", 217, intval($_SESSION['admin']['id']));


// $pdo->query($sql);
// $follow_sid = $pdo->lastInsertId();
// // printf("<!-- %s -->", $order_sid);

// $stmt = $pdo->prepare("INSERT INTO `followed_list`(`sid`, `items_sid`, `user_sid`) VALUES (?, ?, ?)");
...
if ($page < 1) {
    header('Location: item_follow.php');
    exit;
}

$t_sql = "SELECT COUNT(1) FROM followed_list $where";
...
if ($totalPages > 0) {
    if ($page > $totalPages) {
        header('Location: item_follow.php?page=' . $totalPages);
        exit;
    }

    // 讀取分頁的資料
    $sql = sprintf("SELECT `sid`, `items_sid`, `user_sid`, `followed_at` FROM `followed_list` $where ORDER BY sid DESC LIMIT %s, %s", ($page - 1) * $perPage, $perPage);
...
//  列出訂單詳細
$d_sql = sprintf("SELECT`followed_list`.`sid`, `followed_list`.`items_sid` ,`items`.`item_name`, `items`.`item_img`,`items`.`item_price`,`items`.`item_author`,`items`.`item_type` 
FROM `followed_list` 
INNER JOIN `items` 
ON `followed_list`.`items_sid`= `items`.`sid`;");
                ...
                <div class="side_nav">          
                    ...
                    <div class="side_nav_ticket">
                        <div class="hex_icon_wrap">
                         ...
                         <a href="./item-follow.php"><span class="side_nav_text h1">喜好收藏</span></a>
                    </div>
                    <div class="side_nav_ticket">
                       <div class="hex_icon_wrap">
                           <div class="hex_icon">
                ...
                <div class="design-self">
                    <div class="design-hold-item">
                       ...
                       <div class="item-follow-tag d-flex">
                            ...
                            if ($c['parent_sid'] == 0) :
                            ?>
                                    <a href="javascript:selectItems(<?= $c['sid'] ?>)" class="desktop-itemshop-tags-name"><button> # <?= $c['category_name'] ?></button></a>
                            <?php endif; endforeach; ?>
                        </div> 


                        <!-- 喜好收藏 配件清單列表 -->
                        <div class="fitem-follow-items-list-row1 item-follow-items-list-rowtop d-flex f-wrap a-items-star">
                        <!--  -->
                        <?php foreach ($details as $d) :?>
                            <div class="item-follow-item-unit" data-sid="<?= $d['items_sid'] ?>">
                                ...
                                <div class="item-follow-item-name-and-check">
                                    <label class="control control--checkbox"> <?= $d['item_name'] ?>
                                        <input type="checkbox">
                                ...
                                <!-- 配件類型和info icon -->
                                <div
                                    class="item-follow-item-type-and-info d-flex align-items-center justify-content-between">
                                    <a href="#">
                                        <svg viewBox="0 0 16 16" fill="none"
                                            ...
                                            <path
                                                ...
                                                stroke="#D3B572" stroke-linecap="round" stroke-linejoin="round" />
                                            <path d="M8 10.6667V8" stroke="#D3B572" stroke-width="1.5"
                                                stroke-linecap="round" stroke-linejoin="round" />
                                            <path d="M8 5.33301H8.00667" stroke="#D3B572" stroke-width="1.5"
                                                stroke-linecap="round" stroke-linejoin="round" />
                                        </svg>
                                    </a>
                                    <div class="item-follow-item-type"> <?= $d['item_type'] ?></div>
                                </div>
                            </div>
                         <?php endforeach; ?>
                        </div>
                        <!-- <div class="fitem-follow-items-list-row1 item-follow-items-list-rowtop d-flex justify-content-between">
                            <div class="item-follow-item-unit">
                                <div class="item-follow-item-imgwrap">
                                    <img src="./imgs/Itemshop/itemshop_banner_ikehand1.png" alt="">
                                </div>
                                <div class="item-follow-item-name-and-check">
                                    <label class="control control--checkbox">配件名稱
                                        <input type="checkbox" />
                                        <div class="control__indicator"></div>
                                    </label>
                                </div>
...
<?php include __DIR__ . '/parts/html-script.php' ?>
    ...
    //     $('.fitem-follow-items-list-row1').html(data);
